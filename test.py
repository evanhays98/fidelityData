...
def addCompany(content, file, token):
    try:
        requests.post(BASE_URL + '/companies',
    ...
    for i in range(debug, userNum + debug):
        ...
        if companyNum - 1 >= companyAdded:
            for j in range(r1):
                ...
                data += colLogo[r]
                data += '"}'
                addCompany(data, 'Data/BusinessImage/' + imageList[companyAdded], acccess_token)
                companyAdded += 1
...
'''access_token = addUser({
    'userName': 'Evan',
    'email': 'test1d1da11.te3st1@gmail.com',
    'password': 'Password1!'
})
